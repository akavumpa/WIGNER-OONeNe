...
void Plotvncn_OO()
{
  TFile *filereadnch = TFile::Open("avgmult_SuperEvent_OOWS.root");
  TProfile *pfnch = (TProfile *)filereadnch->Get("avgmult");

  const int nmult = (int)pfnch->GetNbinsX();

  // Int_t colors[nflow][ndeta] = {kYellow + 3, kMagenta + 2, kMagenta, kBlue,
  // kCyan + 1, kBlue + 2, kGreen + 2, kRed};
  ...
  TFile *fileWS = TFile::Open("vncn_OOWS.root");
  TFile *fileAC = TFile::Open("vncn_OOAC.root");
  ...
  for (int i = 0; i < nmult; i++)
  {
    ...
    leg1->SetHeader("O#minusO, #sqrt{s_{NN}} = 5.36 TeV, IP-Glasma + MUSIC + iSS + UrQMD");
  ...
  leg1->SetTextSize(0.04);
  leg1->SetHeader(
      Form("#splitline{O#minusO, #sqrt{s_{NN}} = 5.36 TeV, |#eta| < 0.8, %s}{IP-Glasma +  MUSIC + iSS + UrQMD}", deltaetalegg[whichetagap]));
  pad11->cd();
  leg1->Draw();
  pad21->cd();
  leg1->Draw();
  ...
  c1->cd();
  //		leg1->Draw();
  c1->SaveAs(Form("vn_2_V0MSliceComp%s_OO.pdf", deltaetaa[whichetagap]));
